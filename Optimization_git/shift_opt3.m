...
tshift = -.2:.01:.2;
INDMAP = [9 10 8 11 2]; %parameters to optimize

D0 = data_process3(0); %unshifted data
[x0,Init,low,hi] = load_global(D0); % Function returning x0 (the log of the parameters), Init (the initial conditions), 
%all of the variables loaded by load_global are the same for a given data set
ALLPARS = x0;
...
for i = 1:length(tshift)
    ...
    data = data_process3(tshift(i)); %shifted data
    ...
    save(['shift_',num2str(tshift(i)),'_3.mat'])
end
